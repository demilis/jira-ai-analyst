...
-   **`404 Not Found`**:
    -   **원인:** 서버에 연결은 되었지만, 서버가 앱이 요청한 API 주소(`.../rest/api/2/search`)를 모릅니다. **Jira 서버 주소의 기본 경로(Context Path)가 빠졌을 가능성이 매우 높습니다.**
    -   **해결책: 정확한 API 경로 찾기**
        1.  **웹 브라우저로 당신의 Jira에 평소처럼 접속**하세요.
        2.  아무 이슈나 하나 클릭해서 상세 페이지로 이동합니다.
        3.  **브라우저 주소창**을 확인하세요. 주소는 보통 `http://[서버주소]/[추가경로]/browse/[이슈키]` 형태입니다.
