...
이 앱은 역할이 명확하게 구분된 **3단계 구조(Model-View-Controller와 유사)**로 동작합니다. **'식당'**에 비유하면 이해하기 쉽습니다.

### 1단계: **View/Presentation (메뉴판과 테이블)**
- **역할:** 사용자가 보고 상호작용하는 화면입니다.
- **해당 파일:** `src/app/page.tsx`
- **설명:** 사용자는 이곳(웹 브라우저)에서 Jira 접속 정보, 프로젝트 키 등 **주문에 필요한 내용**을 입력하고 '리포트 생성' 버튼을 누릅니다. AI가 만들어준 최종 리포트가 표시되는 곳이기도 합니다.

### 2단계: **Controller/Service (웨이터와 주방 보조)**
- **역할:** 사용자의 요청을 받아 실제 작업을 처리하는 곳으로 전달하고, 그 결과를 다시 사용자 화면으로 가져옵니다.
- **해당 파일:** `src/services/jira-service.ts`, `src/ai/flows/jira-report-flow.ts`의 일부
- **설명:**
    -   **주방 보조 (`jira-service.ts`):** '리포트 생성' 요청을 받으면, 가장 먼저 이 주방 보조가 **Jira의 REST API를 호출**하여 창고(Jira 서버)에서 신선한 **재료(이슈 데이터)**를 가져옵니다.
    -   **웨이터 (`generateJiraReport` 함수):** 가져온 재료와 사용자의 특별 요청(분석 관점)을 모두 모아 메인 셰프(AI)에게 전달하는 역할을 합니다.

### 3단계: **Model (메인 셰프와 레시피)**
- **역할:** 핵심적인 비즈니스 로직을 수행하여 최종 결과물을 만들어냅니다.
- **해당 파일:** `src/ai/flows/jira-report-flow.ts` 의 AI 로직 부분
- **설명:**
    -   **메인 셰프 (Genkit/Google AI):** 웨이터에게 전달받은 재료와 요청사항을 바탕으로, 자신만의 **특별 레시피(AI 프롬프트)**를 사용하여 데이터를 분석하고, 요약하고, 추천 사항을 더해 근사한 **요리(최종 리포트 JSON)**를 만들어냅니다. 이 모든 지능적인 판단은 AI가 담당합니다.

이 구조 덕분에 각 부분은 자신의 역할에만 집중할 수 있어 코드가 더 깔끔하고 유지보수하기 쉬워집니다.
