...
**여러 프로젝트 및 컴포넌트를 한 번에 분석할 수 있습니다!** 각 입력란에 쉼표(`,`)로 여러 값을 입력하면 됩니다. (예: `PROJ, TEST` / `UI, Backend`)
...
- **동작:**
    - 사용자는 웹 브라우저에서 분석할 **Jira 프로젝트 키**(들), **컴포넌트**(들), 분석 관점 등을 입력합니다.
...
-   **`401 Unauthorized` 또는 `403 Forbidden` (인증 실패)**:
    -   **원인:** 서버에 연결은 성공했지만, 인증 정보가 틀렸습니다. 이것은 **코드의 문제가 아니라, 당신이 입력한 값의 문제입니다.**
    -   **해결책:**
        1.  **가장 확실한 방법: 새 API 토큰을 발급받으세요.** Jira 사이트에서 기존 토큰을 삭제하고, 새로운 API 토큰을 발급받아 다시 시도하는 것이 가장 확실합니다.
        2.  **이메일 주소를 다시 확인하세요.** 오타가 없는지 꼼꼼히 살펴보세요.
        3.  **공백을 제거하세요.** API 토큰이나 이메일 주소를 복사/붙여넣기 할 때 앞뒤에 보이지 않는 공백이 포함될 수 있습니다. 메모장 같은 곳에 붙여넣어 확인해보세요.
        4.  **권한을 확인하세요.** 해당 계정이 분석하려는 프로젝트에 접근할 수 있는 권한이 있는지 확인하세요.
