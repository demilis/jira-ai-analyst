...
-   **`401 Unauthorized` 또는 `403 Forbidden` (인증 실패): 최종 점검 가이드**
    ...
    -   **최종 해결을 위한 체크리스트:**
        ...
        5.  **네트워크/방화벽 확인 (최후의 수단):** 만약 위의 모든 방법을 시도했는데도 실패했다면, 회사 내부 네트워크의 방화벽이나 프록시 서버가 외부 스크립트(우리 앱)에서 발생하는 API 요청을 차단하고 있을 수 있습니다. 이 경우, **프로젝트에 함께 추가된 `IT_SUPPORT_REQUEST.md` 파일을 참고하여 사내 IT팀에 문의**하는 것이 가장 빠른 해결책입니다.
