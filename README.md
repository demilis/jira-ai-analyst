...
### 문제 해결: 상세 오류 메시지 확인
만약 여전히 연결 오류가 발생한다면, 앱은 최대한 자세한 오류 메시지를 보여줄 것입니다. 예를 들어:

- **401/403 오류:** '인증 실패' 메시지가 표시되면, Jira 이메일과 API 토큰이 올바른지 다시 확인하세요.
- **404 오류:** 'Not Found' 메시지가 표시되면, `next.config.ts`의 `destination` 주소가 정말 올바른지, 그리고 VPN에 확실히 연결되었는지 확인하세요.
- **시간 초과(Timeout) 오류:** `destination` 주소는 맞지만 서버가 응답이 없거나 네트워크가 매우 느린 경우입니다.

이 상세 메시지는 문제의 원인을 찾는 데 결정적인 단서가 될 것입니다.
