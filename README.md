...
- **동작:**
    - 사용자는 웹 브라우저에서 분석할 **Jira 서버 주소**, **인증 정보(이메일/사용자이름, API 토큰)**, **프로젝트 키**(들), **컴포넌트**(들), 분석 관점 등을 입력합니다.
...
- **동작:**
    - **Jira 데이터 요청 (`src/services/jira-service.ts`):** Client의 요청에 따라 Jira 서버의 REST API를 호출하여 분석에 필요한 원본 이슈 데이터를 가져옵니다. **사용자가 UI에 입력한 인증 정보를 직접 사용하며, 서버에서는 어떠한 정보도 저장하거나 하드코딩하지 않습니다.**
...
- **Jira 서버:** 이슈 데이터의 원천 소스입니다. App Server의 요청에 따라 데이터를 제공하고, **API 토큰의 유효성을 검증하는 최종 주체**입니다.
...
- **목표:** `npm run dev` 명령어로 앱을 실행하여, UI에 직접 입력한 인증 정보를 통해 회사 내부망에 있는 Jira 서버의 데이터를 실제로 분석하고 결과를 확인합니다.
...
### 4단계: 환경 변수 설정 (Google AI API 키)
Jira 접속 정보는 UI에서 직접 입력하지만, Google AI를 사용하기 위한 API 키는 **환경 변수 파일**에 설정해야 합니다.

1.  프로젝트의 최상위 폴더(루트)에 `.env.local` 이라는 파일을 새로 만드세요.
2.  아래 내용을 복사하여 파일에 붙여넣고, 당신의 Google AI API 키 값으로 변경하세요.
...
1. **VPN 연결:** 먼저, 분석하려는 Jira 서버에 접속할 수 있도록 회사 VPN에 연결하세요.
2. **개발 서버 실행:** 터미널에서 아래 명령어를 실행하면 로컬 개발 서버가 시작됩니다.
   ```bash
   npm run dev
   ```
3. **앱 접속:** 웹 브라우저에서 `http://localhost:3000` 주소로 접속하여 앱을 사용합니다. 필요한 모든 정보는 UI에서 직접 입력합니다.
...
-   **`네트워크 오류: ... 연결할 수 없습니다`**:
    ...
    -   **해결책:**
        ...
        2.  UI에 입력한 **Jira 주소**에 오타가 없는지 다시 한번 꼼꼼히 확인하세요.

-   **`404 Not Found`**:
    -   **원인:** 서버에 연결은 되었지만, API 주소를 모르는 경우. **Jira 서버의 기본 경로(Context Path, 예: `/issue`)가 빠졌을 가능성이 매우 높습니다.**
    -   **해결책:** UI의 'Jira 주소' 입력 필드에, 브라우저에서 접속하는 **전체 주소**를 정확히 입력했는지 다시 확인하세요. (예: `http://jira.lge.com/issue`)

-   **`400 Bad Request`**:
    -   **원인:** Jira 서버가 당신의 요청을 이해하지 못했습니다. 보통 JQL 쿼리에 문법 오류가 있을 때 발생합니다.
    -   **해결책:** 입력한 **프로젝트 키** 또는 **컴포넌트 이름** 중에 존재하지 않는 값이 있거나 오타가 있는지 확인해보세요.

-   **`401 Unauthorized` 또는 `403 Forbidden` (인증 실패): 최종 점검 가이드**
    -   **원인:** 이것은 **코드가 아닌, 당신이 입력한 인증 정보나 계정/네트워크 정책의 문제일 가능성이 99%입니다.** Jira 서버가 당신의 신원 확인을 거부한 상태입니다.
    -   **최종 해결을 위한 체크리스트:**
        1.  **새로운 API 토큰 발급 (가장 확실한 방법):** Jira 사이트에서 기존 토큰을 모두 **Revoke(폐기)**하고, **완전히 새로운 API 토큰을 발급**받아 다시 시도해보세요. 이것이 대부분의 문제를 해결합니다.
        2.  **공백 문자 확인:** API 토큰이나 이메일 주소를 복사/붙여넣기 할 때 앞뒤에 보이지 않는 공백이 포함될 수 있습니다. 메모장 같은 곳에 붙여넣어 공백이 없는지 확인하세요.
        3.  **이메일 vs 사용자 이름:** 'Jira 계정 이메일' 필드에 **이메일 주소** 대신, Jira 로그인 시 사용하는 **사용자 이름(아이디)**을 입력해서 시도해보세요. (예: `seungho.baek` )
        4.  **계정 권한 확인:** 현재 당신의 계정이 분석하려는 프로젝트에 접근할 수 있는 권한을 가지고 있는지 Jira 사이트에서 직접 확인해보세요.
        5.  **네트워크/방화벽 확인 (최후의 수단):** 만약 위의 모든 방법을 시도했는데도 실패했다면, 회사 내부 네트워크의 방화벽이나 프록시 서버가 외부 스크립트(우리 앱)에서 발생하는 API 요청을 차단하고 있을 수 있습니다. 사내 IT/보안팀에 "Node.js 스크립트 환경에서 Jira REST API로 Basic 인증 요청 시 401 오류가 발생합니다. 관련 정책이 있는지 확인 부탁드립니다." 라고 문의해 볼 수 있습니다.

-   **`시간 초과(Timeout) 오류`**:
    -   **원인:** Jira 서버가 응답이 없거나 네트워크가 매우 느린 경우입니다.
