...
## Firebase 개발 환경과 로컬 실행 환경

이 프로젝트는 두 가지 환경을 목적에 맞게 사용합니다.

-   **Firebase Studio (AI 개발 환경):** 이 곳은 AI와 소통하며 앱의 핵심 로직과 UI를 빠르게 개발하고 프로토타이핑하는 데 사용됩니다. AI의 강력한 코드 생성 및 수정 능력을 마음껏 활용할 수 있습니다.
-   **로컬 PC (내부망 실행 환경):** 개발이 완료된 앱을 **사내망 Jira와 같은 내부 서버에 연결하여 실제 데이터를 분석**하고 싶을 때 사용합니다. `next.config.ts`의 프록시 설정 덕분에, 로컬에서 앱을 실행하면 안전하게 내부 서버와 통신할 수 있습니다.

> **중요:** Firebase에 배포된 앱은 공개 인터넷에 위치하므로, 보안 정책상 회사 내부망에 있는 Jira 서버에는 절대 직접 접속할 수 없습니다. 내부망 접속은 반드시 로컬 PC 환경에서 실행해야 합니다.
