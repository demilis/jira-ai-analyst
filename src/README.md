...
이 애플리케이션은 역할과 책임이 명확하게 분리된 아키텍처를 따릅니다. 각 구성 요소가 어떻게 상호작용하는지 이해하면 전체 시스템을 파악하는 데 도움이 됩니다.

### 1. Client (프론트엔드 - `src/app/page.tsx`)
- **역할:** 사용자와의 모든 상호작용을 담당하는 UI(User Interface) 계층입니다.
- **동작:**
  - 사용자가 브라우저에서 Jira 인스턴스 정보, 프로젝트 키, 분석 관점 등을 입력합니다.
  - '리포트 생성' 버튼을 클릭하면, 입력된 정보를 App Server로 전송하여 분석을 요청합니다.
  - 분석이 완료되면 App Server로부터 최종 리포트(JSON 형식)를 받아 사용자에게 시각적으로 보여줍니다. (차트, 요약 텍스트, 아코디언 등)

### 2. App Server (백엔드 - Next.js)
- **역할:** Client의 요청을 받아 핵심 로직을 수행하고 외부 서비스와 통신하는 중간 계층입니다.
- **동작:**
  - **데이터 요청 (`src/services/jira-service.ts`):** Client로부터 요청을 받으면, 이 서비스가 Jira 서버의 REST API를 호출하여 필요한 이슈 데이터를 가져옵니다. 이 과정은 App Server가 Jira 서버의 '클라이언트' 역할을 하는 것과 같습니다.
  - **AI 분석 요청 (`src/ai/flows/jira-report-flow.ts`):** Jira로부터 가져온 원본 데이터를 Genkit AI(Google AI)에 전달하여 분석을 지시합니다. 여기서 AI는 복잡한 데이터를 의미 있는 정보로 가공하는 '두뇌' 역할을 수행합니다.
  - **응답 반환:** AI가 분석을 완료하고 구조화된 리포트(JSON)를 생성하면, App Server는 이를 다시 Client에게 전달합니다.

### 3. External Services (외부 서비스)
- **Jira 서버:** 이슈 데이터의 원천 소스입니다. App Server의 요청에 따라 데이터를 제공하는 역할을 합니다.
- **Google AI (Genkit):** App Server의 요청에 따라 Jira 데이터를 분석하고, 요약하며, 실행 가능한 인사이트를 생성하는 지능형 서비스입니다.

---

### 개발 과정과 Model Context Protocol (MCP)

이 애플리케이션의 코드는 **Firebase Studio의 AI 코딩 파트너**와 협력하여 개발됩니다. 이때 AI 파트너는 코드를 직접 수정하지 않고, **Model Context Protocol (MCP)** 이라는 명확한 규칙을 따릅니다.

- **MCP란?** AI가 파일 변경 사항을 제안하기 위해 사용하는 **XML 기반의 약속(Protocol)** 입니다.
- **동작 방식:**
  1.  개발자가 "아키텍처 설명을 바꿔줘" 와 같이 변경을 요청합니다.
  2.  AI는 변경이 필요한 파일의 **전체 최종본**을 포함하는 XML 구조의 '변경 계획서'를 생성합니다.
  3.  시스템은 이 XML 계획서를 해석하여 실제 파일에 변경 사항을 안전하고 정확하게 적용합니다.

이러한 방식은 AI와 개발자 간의 협업에서 투명성과 신뢰성을 보장하며, Client, App Server, AI 로직 등 이 애플리케이션의 모든 구성요소는 이 MCP를 통해 생성되고 수정됩니다.
