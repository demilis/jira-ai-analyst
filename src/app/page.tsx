...
export default function Home() {
  ...
  const [jiraAuth, setJiraAuth] = useState<JiraAuth>({
    instanceUrl: 'http://jira.lge.com/issue',
    email: '',
    apiToken: '',
  });
  ...
  const handleGenerateReport = async () => {
    ...
    try {
        ...
        if (issueDataArray.length < 2) { 
            toast({
                variant: 'destructive',
                title: 'Jira에서 이슈를 가져오지 못했습니다.',
                description: '입력하신 인증 정보와 프로젝트/컴포넌트 키가 올바른지, 해당 프로젝트에 접근 가능한지 확인해주세요.',
            });
            setIsLoading(false);
            return;
        }
      ...
      <Card className="w-full max-w-3xl">
        ...
        <CardContent>
            <Alert variant="destructive" className="mb-6">
                ...
                <AlertDescription>
                   로컬 PC에서 실행 시: 내부망 Jira에 접속하려면 회사 VPN에 반드시 연결해야 합니다.
                </AlertDescription>
            </Alert>
            <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                        <Label htmlFor="instanceUrl">Jira 주소 (Context Path 포함)</Label>
                        <Input
                            id="instanceUrl"
                 ...
                 <div className="space-y-2">
                    ...
                    <Input
                        id="apiToken"
                        type="password"
                        placeholder="Jira에서 발급받은 API 토큰을 붙여넣으세요"
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                        <Label htmlFor="jiraProjectKey">Jira 프로젝트 키</Label>
                        <Input
                            id="jiraProjectKey"
                            placeholder="분석할 프로젝트 키 (예: PROJ, TEST)"
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="jiraComponent">컴포넌트 (선택 사항)</Label>
                        <Input
                            id="jiraComponent"
                            placeholder="특정 컴포넌트 이름 (예: UI, Backend)"
                            value={jiraComponent}
          ...
          <div className="mt-6 space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              AI가 생성할 요약 및 조치 항목의 초점을 지정합니다. (예: "지난 주", "홍길동 담당 이슈")
            </p>
          </div>
      ...
      {isLoading && (
        <Card className="w-full max-w-3xl mt-6">
          <CardContent className="p-6 flex flex-col items-center justify-center">
             ...
             <p className="text-muted-foreground">AI가 Jira 이슈를 분석하고 있습니다. 잠시만 기다려주세요...</p>
          </CardContent>
        </Card>
      )}
        ...
        <Card className="w-full max-w-3xl mt-6">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>요청하신 관점에 맞춰 분석이 완료되었습니다.</CardDescription>
            </div>
