...
const JiraReportInputSchema = z.object({
  issuesData: z.string().describe("A stringified JSON array of arrays representing the Jira issues. The first inner array is the header row."),
...
const JiraReportOutputSchema = z.object({
  summary: z.string().describe("A high-level summary of all the issues provided. It should mention the total number of issues, how many are open, in progress, and done. Mention any noticeable trends or bottlenecks. This MUST be in KOREAN."),
  priorityActions: z.array(z.string()).describe("A bulleted list of the most critical actions to take, based on issue priority, status, and content. Max 3-5 items. This MUST be in KOREAN."),
  issueBreakdown: z.array(IssueBreakdownItemSchema).describe("A detailed breakdown of each individual issue. All string values MUST be in KOREAN unless it's a key."),
...
3.  **KOREAN LANGUAGE:** All human-readable text output (summary, priorityActions, recommendations, etc.) MUST be in natural KOREAN.
