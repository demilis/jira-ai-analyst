...
### 문제 해결: 401 인증 실패 - 최종 점검 가이드
만약 모든 설정을 마쳤는데도 계속 `401 Unauthorized` 오류가 발생한다면, 원인은 99.9% `.env.local` 파일에 입력한 값에 있습니다. 아래 체크리스트를 순서대로 확인해 보세요.

1.  **새로운 API 토큰을 발급받으셨나요? (가장 확실한 방법)**
    *   기존 토큰에 오타가 있거나, 복사 과정에서 실수가 있었을 수 있습니다.
    *   **지금 바로 [Atlassian API 토큰 관리 페이지](https://id.atlassian.com/manage-profile/security/api-tokens)에 방문하여 새 토큰을 만드세요.**
    *   **[Create]** 버튼을 눌러 생성된 토큰 전체를 정확하게 복사한 후, `JIRA_API_TOKEN` 값에 붙여넣으세요.

2.  **이메일 주소가 정확한가요?**
    *   Jira 프로필 페이지에 등록된 이메일 주소와 `JIRA_EMAIL` 값이 100% 일치하는지 확인하세요.

3.  **복사/붙여넣기 실수는 없었나요?**
    *   토큰이나 이메일을 붙여넣을 때, 앞이나 뒤에 눈에 보이지 않는 공백(` `)이 포함되는 경우가 매우 많습니다. 값을 다시 한번 확인해 보세요.

4.  **서버를 재시작했나요?**
    *   `.env.local` 파일을 수정한 후에는 **반드시** 개발 서버를 중지(`Ctrl+C`)했다가 다시 시작(`npm run dev`)해야 합니다.

이 네 가지를 모두 확인하시면 문제는 반드시 해결됩니다.
